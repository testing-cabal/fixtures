...
if not getattr(inspect, 'signature', None):
    from funcsigs import signature as inspectsignature
else:
...
def _setattr(obj, name, value, check_value=False):
    ...
    if py2:
        ...
        # definition so we can check the type.
        # If 'value' is callable it's a classmethod/staticmethod and shouldn't
        # be rewrapped.
        if (isinstance(attr_to_check, types.FunctionType)) and callable(value):
        ...
        # If 'value' is callable it's a classmethod/staticmethod and shouldn't
        # be rewrapped.
        sig = inspectsignature(attr_to_check)
        if 'self' not in sig.parameters and callable(value):
            value = staticmethod(value)
    setattr(obj, name, value)
