...
NEW_PY39_CLASSMETHOD = sys.version_info[:2] == (3, 10) and not hasattr(
...
class TestMonkeyPatch(testtools.TestCase, TestWithFixtures):
    def test_patch_and_restore(self):
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.reference", 45)
    ...
    def test_patch_missing_attribute(self):
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.new_attr", True)
    ...
    def test_delete_existing_attribute(self):
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.reference",
            MonkeyPatch.delete,
        )
        self.assertEqual(23, reference)
        fixture.setUp()
    ...
    def test_delete_missing_attribute(self):
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.new_attr",
            MonkeyPatch.delete,
        )
        self.assertFalse("new_attr" in globals())
        fixture.setUp()
    ...
    def test_patch_staticmethod_with_staticmethod(self):
        ...
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.C.foo_static",
            D.bar_static,
        )
        with fixture:
            C.foo_static()
    ...
    def test_patch_staticmethod_with_classmethod(self):
        ...
        oldmethod_inst = C().foo_static
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.C.foo_static", D.bar_cls
        )
        with fixture:
            C.foo_static()
            C().foo_static()
    ...
    def test_patch_staticmethod_with_boundmethod(self):
        oldmethod = C.foo_static
        oldmethod_inst = C().foo_static
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo_static", D().bar)
        with fixture:
            C.foo_static()
            C().foo_static()
        self._check_restored_static_or_class_method(
    ...
    def test_patch_classmethod_with_staticmethod(self):
        ...
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.C.foo_cls",
            D.bar_static_args,
        )
        with fixture:
            (cls,) = C.foo_cls()
    ...
    def test_patch_classmethod_with_function(self):
        oldmethod = C.foo_cls
        oldmethod_inst = C().foo_cls
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo_cls", fake)
        with fixture:
            (cls,) = C.foo_cls()
            self.expectThat(cls, Is(C))
            (cls, arg) = C().foo_cls(1)
    ...
    def test_patch_function_with_staticmethod(self):
        ...
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.fake_no_args",
            D.bar_static,
        )
        with fixture:
            fake_no_args()
    ...
    def test_patch_function_with_classmethod(self):
        oldmethod = fake_no_args
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.fake_no_args", D.bar_cls
        )
        with fixture:
            fake_no_args()
        self.assertEqual(oldmethod, fake_no_args)
    ...
    def test_patch_function_with_boundmethod(self):
        oldmethod = fake_no_args
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.fake_no_args", D().bar)
        with fixture:
            fake_no_args()
        self.assertEqual(oldmethod, fake_no_args)
    ...
    def test_patch_boundmethod_with_staticmethod(self):
        ...
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.INST_C.foo",
            D.bar_static,
        )
        with fixture:
            INST_C.foo()
    ...
    def test_patch_boundmethod_with_classmethod(self):
        oldmethod = INST_C.foo
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.INST_C.foo", D.bar_cls)
        with fixture:
            INST_C.foo()
        self.assertEqual(oldmethod, INST_C.foo)
    ...
    def test_patch_boundmethod_with_boundmethod(self):
        oldmethod = INST_C.foo
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.INST_C.foo", D().bar)
        with fixture:
            INST_C.foo()
        self.assertEqual(oldmethod, INST_C.foo)
        sentinel = object()
    ...
    def test_patch_unboundmethod_with_staticmethod(self):
        oldmethod = C.foo
        fixture = MonkeyPatch(
            "tests._fixtures.test_monkeypatch.C.foo",
            D.bar_static_args,
        )
        with fixture:
    ...
    def test_patch_unboundmethod_with_classmethod(self):
        oldmethod = C.foo
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo", D.bar_cls_args)
        with fixture:
            c = C()
    ...
    def test_patch_unboundmethod_with_function(self):
        oldmethod = C.foo
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo", fake)
        with fixture:
            c = C()
    ...
    def test_patch_unboundmethod_with_boundmethod(self):
        oldmethod = C.foo
        d = D()
        fixture = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo", d.bar_two_args)
        with fixture:
            c = C()
    ...
    def test_double_patch_instancemethod(self):
        ...
        fixture1 = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo", fake)
        fixture2 = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo", fake2)
        with fixture1:
            with fixture2:
                C().foo()
        self.assertEqual(oldmethod, C.foo)
    ...
    def test_double_patch_classmethod(self):
        oldmethod = C.foo_cls
        oldmethod_inst = C().foo_cls
        fixture1 = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo_cls", fake)
        fixture2 = MonkeyPatch("tests._fixtures.test_monkeypatch.C.foo_cls", fake2)
        with fixture1:
            with fixture2:
                C.foo_cls()
                C().foo_cls()
