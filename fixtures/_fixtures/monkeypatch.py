...
def _coerce_values(obj, name, new_value, sentinel):
    ...
    # If new_value is not callable it is either not a method or already wrapped
    # with staticmethod/classmethod. Either way it needs no coercion.
    if callable(new_value):
        ...
        if isinstance(old_value, classmethod):
            if hasattr(new_value, '__func__'):
                # If new_value is a bound classmethod pull the function object
                new_value = classmethod(new_value.__func__)
            else:
